...
const DataListView = ({ product, isSelect, collect, onCheckItem,categories,change,setchange,index }) => {
  ...
  const edit = (e) => {
    ...
    axios
      .post('/api/edit-user', data, { params: { id: e } })
      .then((res) => {
        console.log(res);
        if (res.data.success) {
          setchange(!change)
        ...
        <ModalBody>
          <div className="d-flex align-items-center justify-content-center flex-column p-3">
            ...
            />:<img
              ...
              alt=""
            />}
            <Label className="font-weight-bold" htmlFor="profile">
              <IntlMessages id="Change Picture" />
            </Label>
            <Input onChange={onChangePicture} style={{ display: 'none' }} id="profile" type="file" />
          </div>
          <Row className='p-1'>
            <Col className="sm-6">
              <Label>
